...
async function main() {
  ...
  // --- VIVO POS: a street-seller catalog + a couple of logged sales -------
  const posCatalogData = [
    { id: 'pos-item-seed-1', sellerId: maria.id, name: 'Elote (grilled corn)', price: 1.5 },
    { id: 'pos-item-seed-2', sellerId: maria.id, name: 'Fresh coconut water', price: 2.0 },
    { id: 'pos-item-seed-3', sellerId: maria.id, name: 'Artisan friendship bracelet', price: 4.0 },
    { id: 'pos-item-seed-4', sellerId: demo.id, name: 'Homemade tamales (dozen)', price: 8.0 },
    { id: 'pos-item-seed-5', sellerId: demo.id, name: 'Hand-poured candle', price: 6.5 },
  ];
  const posItems: Record<string, { id: string; price: number }> = {};
  for (const p of posCatalogData) {
    const item = await db.posCatalogItem.upsert({
      where: { id: p.id },
      update: {},
      create: { id: p.id, sellerId: p.sellerId, name: p.name, price: p.price },
    });
    posItems[p.id] = { id: item.id, price: p.price };
  }
  ...
  const posSaleData = [
    {
      id: 'pos-sale-seed-1',
      sellerId: maria.id,
      receiptCode: 'DEMO0001',
      daysBack: 2,
      customerPhone: null as string | null,
      lines: [
        { itemId: 'pos-item-seed-1', quantity: 3 },
        { itemId: 'pos-item-seed-2', quantity: 2 },
      ],
    },
    {
      id: 'pos-sale-seed-2',
      sellerId: demo.id,
      receiptCode: 'DEMO0002',
      daysBack: 1,
      customerPhone: null as string | null,
      lines: [{ itemId: 'pos-item-seed-4', quantity: 1 }],
    },
  ];
  for (const s of posSaleData) {
    const items = s.lines.map((l) => posItems[l.itemId]);
    const totalAmount = s.lines.reduce((sum, l) => sum + posItems[l.itemId].price * l.quantity, 0);
    await db.posSale.upsert({
      where: { id: s.id },
      update: {},
      create: {
        id: s.id,
        sellerId: s.sellerId,
        clientSaleId: `seed-${s.id}`,
        ...
        items: {
          create: s.lines.map((l) => ({
            ...
            name: posCatalogData.find((p) => p.id === l.itemId)!.name,
            quantity: l.quantity,
            unitPrice: posItems[l.itemId].price,
          })),
        },
      },
    });
