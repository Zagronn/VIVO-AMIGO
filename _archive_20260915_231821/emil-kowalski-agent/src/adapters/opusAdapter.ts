...
export async function callOpusDesignEngine(componentCode: string): Promise<string> {
	if (!componentCode.trim()) throw new Error('componentCode is required');
	if (!process.env.ANTHROPIC_API_KEY?.trim()) throw new Error('ANTHROPIC_API_KEY is required for Opus pipeline');
	...
	if (!block || block.type !== 'text' || !block.text.trim()) throw new Error('Opus provider returned no design output');
	return block.text;
}
