...
export const TESTE_SKILL_SYSTEM_PROMPT = `
...
function requiredComponentCode(componentCode: string): string {
  if (typeof componentCode !== 'string' || componentCode.trim() === '') throw new Error('componentCode is required');
  return componentCode;
}

export async function generateComponentTests(componentCode: string): Promise<string> {
  if (!process.env.GEMINI_API_KEY?.trim()) throw new Error('GEMINI_API_KEY is required to generate component tests');
  const response = await new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY }).models.generateContent({
    ...
    config: { systemInstruction: TESTE_SKILL_SYSTEM_PROMPT, temperature: 0.2 }
  });
  if (!response.text?.trim()) throw new Error('Gemini returned an empty test suite');
