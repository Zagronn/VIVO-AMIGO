  ...
  return (
    <main className="min-h-screen bg-[#F8F9FA] text-[#25262C]">
      ...
      <header className="border-b border-gray-200 bg-white">
        ...
        <nav className="border-t border-gray-100 px-4" aria-label="Servicios VIVO"><div className="mx-auto flex max-w-7xl gap-6 overflow-x-auto py-3 text-xs font-bold whitespace-nowrap"><Link href="/wallet" className="text-[#EB5A00]">VIVO PAY</Link><a href="https://cargovivo.com" className="text-[#2563EB]">VIVO SHIP</a><a href="#destacados" className="text-[#7C3AED]">VIVO ADS</a><Link href="/business" className="text-[#7C3AED]">VIVO BUSINESS</Link><a href="#soporte" className="text-[#166534]">VIVO SUPPORT</a><a href="#categorias" className="text-gray-600">Categorías</a></div></nav>
      </header>

      <section className="mx-auto grid max-w-7xl items-center gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[1fr_.9fr] lg:py-20">
        <div><p className="mb-4 text-xs font-extrabold tracking-[0.18em] text-[#EB5A00]">EL MERCADO DIGITAL DE GUATEMALA</p><h1 className="max-w-2xl text-5xl font-extrabold leading-[1.02] tracking-tight text-[#25262C] sm:text-6xl">Todo lo que buscas, <span className="text-[#FF6A00]">en un solo lugar</span></h1><p className="mt-6 max-w-xl text-base leading-7 text-gray-600">Compra, vende y conecta con pagos protegidos, envíos rápidos y acompañamiento real en cada operación.</p><div className="mt-8 flex flex-wrap gap-3"><motion.a href="#destacados" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} transition={spring} className="inline-flex items-center gap-2 rounded-lg bg-[#FF6A00] px-5 py-3 font-bold text-white shadow-sm hover:bg-[#EB5A00]">Comenzar ahora <ChevronRight size={18} /></motion.a><motion.a href="#vender" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} transition={spring} className="rounded-lg border border-[#FF6A00] px-5 py-3 font-bold text-[#EB5A00] hover:bg-orange-50">Vender ahora</motion.a></div></div>
        <div className="relative mx-auto aspect-square w-full max-w-md"><div className="absolute inset-7 rounded-full border-[18px] border-[#FF6A00]/25 bg-[#EDF2F7]" /><div className="absolute inset-14 grid place-items-center overflow-hidden rounded-full bg-[#25262C] shadow-xl"><VivoBrandLogo compact href={null} light /></div><FloatingIcon icon={ShoppingBag} className="left-2 top-16" color="text-[#FF6A00]" /><FloatingIcon icon={Car} className="right-4 top-10" color="text-[#2563EB]" /><FloatingIcon icon={Smartphone} className="bottom-12 left-8" color="text-[#7C3AED]" /><FloatingIcon icon={Home} className="bottom-4 right-12" color="text-[#16A34A]" /></div>
      </section>
