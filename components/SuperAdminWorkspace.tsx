...
function Panel({ tab, pending, onResolve, blockedIps, onBlockIp }: { tab: Tab; pending: string[]; onResolve: (item: string) => void; blockedIps: string[]; onBlockIp: () => void }) {
  if (tab === 'Overview') return <><Metrics values={[['System telemetry', 'Operational'], ['Agent monitoring', '250 ready'], ['Open reviews', String(pending.length)]]} /><Queue items={pending} onResolve={onResolve} /></>;
  ...
  return <><Metrics values={[['Blocked IPs', String(blockedIps.length)], ['Security alerts', '0'], ['Last audit', 'Just now']]} /><section className="vivo-glass-panel mt-5 rounded-xl p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-lg font-extrabold">Access controls</h3><p className="mt-1 text-sm text-white/55">Local preview block list. Server enforcement requires a security provider.</p></div><button type="button" onClick={onBlockIp} className="inline-flex items-center gap-2 rounded-lg border border-red-300/35 px-3 py-2 text-xs font-bold text-red-200"><Ban size={15} />Block IP</button></div><div className="mt-4 space-y-2">{blockedIps.length ? blockedIps.map((ip) => <p key={ip} className="rounded-lg border border-white/10 bg-black/15 p-3 text-sm">{ip}</p>) : <p className="rounded-lg border border-white/10 bg-black/15 p-3 text-sm text-white/55">No local IP blocks configured.</p>}</div></section></>;
}
