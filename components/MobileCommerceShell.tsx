...
export const MobileCommerceShell = () => {
  ...
  return <main className="min-h-screen bg-[#F8F9FA] pb-24 text-[#111111] md:pb-8">
    ...
    <AnimatePresence mode="wait" initial={false}><motion.div key={tab} layout initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={spring} className="mx-auto max-w-5xl overflow-hidden px-4">
      {tab === 'home' && <><ReturnGreeting /><CorporateBillboard {...homeOffer} targetCategory="VEHICLES" /><motion.section layout className="mt-5 rounded-3xl bg-[#111111] p-6 text-white shadow-xl"><span className="rounded-full bg-[#16A34A] px-2 py-1 text-[10px] font-extrabold uppercase">{t.security}</span><h1 className="mt-5 text-4xl font-extrabold leading-tight tracking-tight">{t.title}</h1><p className="mt-3 max-w-md text-sm text-white/60">{t.subtitle} <strong className="text-[#FF6A00]">Llevamos Vidas.</strong></p><motion.button type="button" onClick={() => setTab('search')} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} transition={spring} className="mt-6 rounded-full bg-[#FF6A00] px-5 py-3 text-sm font-extrabold text-white">{t.explore} →</motion.button></motion.section><section className="mt-6"><div className="mb-3 flex items-center justify-between"><h2 className="text-xl font-extrabold tracking-tight">{language === 'es' ? 'Oportunidades destacadas' : 'Featured opportunities'}</h2><span className="text-xs font-bold text-black/40">3</span></div><div className="grid gap-3 sm:grid-cols-3">{listings.map((listing) => <ListingCard key={listing.title} listing={listing} trustedLabel={t.verified} />)}</div></section></>}
...
function ListingCard({ listing, trustedLabel }: { listing: typeof listings[number]; trustedLabel: string }) {
  return <motion.article layout whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} transition={spring} className={cn('rounded-2xl border border-black/5 bg-white/70 p-4 shadow-sm backdrop-blur-md transition-shadow duration-300 hover:shadow-lg hover:shadow-black/5 dark:border-white/10 dark:bg-zinc-900/70 dark:hover:shadow-white/5')}><div className="grid h-32 place-items-center rounded-xl bg-[#FFF7F1] text-4xl font-extrabold text-[#FF6A00]">{listing.mark}</div><p className="mt-3 text-[10px] font-bold uppercase text-black/40">{listing.category}</p><h3 className="mt-1 font-extrabold tracking-tight">{listing.title}</h3><p className="mt-2 text-lg font-extrabold">{listing.price}</p>{listing.trusted && <span className="mt-3 inline-flex rounded-full bg-[#DCFCE7] px-2 py-1 text-[10px] font-bold text-[#166534]">✓ {trustedLabel}</span>}</motion.article>;
}